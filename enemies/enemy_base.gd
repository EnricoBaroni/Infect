...
func _get_effective_speed() -> float:
	...
	if _status_states.has("chained"):
		var chained_state: Dictionary = _status_states["chained"]
		effective *= clamp(float(chained_state["magnitude"]), 0.0, 1.0)
	return effective

func is_baited() -> bool:
	return _status_states.has("bait")
