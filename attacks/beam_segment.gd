...
func setup_segment(attack_data: AttackData, segment_index: int) -> void:
	...
	var crit_chance: float = clampf(attack_data.crit_chance, 0.0, 1.0)
	var crit_multiplier: float = max(1.0, attack_data.crit_multiplier)
	...
	var resolved_damage: float = attack_data.damage
	...
	var visual_scale: float = clamp(1.0 - segment_index * 0.04, 0.6, 1.0)
...
func _spawn_explosion_projectiles(origin: Vector2) -> void:
	...
	var count: int = max(1, _explosion_spawn_count)
	var step: float = 0.0
