...
const BULLET_SCENE = preload("uid://cwe1h0ebu2ech")
const TEAR_BREAK_EFFECT = preload("uid://xr1qsl84tdtd")
...
func _on_body_entered(body: Node2D) -> void:
	...
	if remaining_bounces > 0:
		remaining_bounces -= 1
		var reflected_direction = -direction.normalized()
		if bounce_random_angle_degrees > 0.0:
			var random_angle = deg_to_rad(randf_range(-bounce_random_angle_degrees, bounce_random_angle_degrees))
...
func _spawn_fracture_projectiles(origin: Vector2) -> void:
	...
	for idx in fragment_count:
		...
		var fragment = BULLET_SCENE.instantiate()
...
func _spawn_explosion_projectiles(origin: Vector2) -> void:
	...
	var count: int = maxi(1, explosion_spawn_count)
	var step := 0.0
	var start_angle := 0.0
	if full_circle:
...
func _apply_homing(delta: float) -> void:
	...
	var steer_factor = clamp(homing_strength * delta, 0.0, 1.0)
