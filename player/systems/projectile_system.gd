...
func _spawn_tear_projectile(
	...
	for angle_deg in attack_data.appended_shot_offsets:
		var appended_data: AttackData = _clone_attack_data(attack_data)
		appended_data.appended_shot_offsets = []
		appended_data.direction = attack_data.direction.rotated(deg_to_rad(angle_deg))
		for direction in _build_attack_directions(appended_data):
			var shot_data: AttackData = _clone_attack_data(appended_data)
			shot_data.direction = direction
			var bullet_instance := bullet_scene.instantiate()
			bullet_instance.global_position = spawn_position
			bullet_instance.setup_attack(shot_data)
			get_tree().current_scene.add_child(bullet_instance)
...
func _build_attack_directions(attack_data: AttackData) -> Array[Vector2]:
	...
	var total_split: int = max(0, attack_data.split_count)
	var spread: float = attack_data.split_spread_degrees
