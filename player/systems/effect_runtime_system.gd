...
func _exit_tree() -> void:
	...
	for item in _active_instances_by_item.keys():
...
func _register_item(item: ItemData) -> void:
	...
	# Safety check: prevent orphaned runtimes if same ItemData reference registered twice
	...
	for effect in item.effects:
		...
		if effect is not ReactiveEffectData:
			continue
		var owner_player: Player = get_parent() as Player
		if owner_player == null:
			continue

		var runtime_instance: ReactiveEffectInstance = effect.create_runtime(owner_player)
