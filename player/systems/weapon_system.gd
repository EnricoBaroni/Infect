...
func _apply_charge_scaling(attack_data: AttackData) -> void:
	...
	var min_time: float = maxf(0.01, attack_data.charge_min_time)
	var max_time: float = maxf(min_time, attack_data.charge_max_time)
	var ratio := clampf((_charge_elapsed - min_time) / max(0.001, max_time - min_time), 0.0, 1.0)

	var damage_scale := lerpf(attack_data.charge_damage_scale_min, attack_data.charge_damage_scale_max, ratio)
	var speed_scale := lerpf(attack_data.charge_speed_scale_min, attack_data.charge_speed_scale_max, ratio)
	var range_scale := lerpf(attack_data.charge_range_scale_min, attack_data.charge_range_scale_max, ratio)
	var size_scale := lerpf(attack_data.charge_size_scale_min, attack_data.charge_size_scale_max, ratio)
	...
	var split_bonus_f := lerpf(float(attack_data.charge_split_min), float(attack_data.charge_split_max), ratio)
