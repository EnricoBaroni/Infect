...
func _physics_process(delta: float) -> void:
	...
	var center: float = float(_total - 1) * 0.5
	var offset_angle: float = (float(_slot) - center) * spread
	var desired: Vector2 = _player.global_position + Vector2(-_follow_distance, 0).rotated(offset_angle)
...
func _block_enemy_projectiles() -> void:
	for area in hitbox.get_overlapping_areas():
		...
		if not (area is Area2D):
			continue
		if not bool(area.get("enemy_shot")):
			continue
