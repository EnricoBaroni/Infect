...
class_name GainDropsOnKillEffect

@export var drops_per_kill: int = 1
...
func create_runtime(player: Player) -> ReactiveEffectInstance:
	var runtime := GainDropsOnKillRuntime.new()
	runtime.configure(player, drops_per_kill, infected_bonus, proc_chance, luck_to_full, max_proc_chance)
	return runtime
